...
def create_modules(module_defs, is_encoder):
    """
    ...
    for i, module_def in enumerate(module_defs):
        ...
        if module_def["type"] == "convolutional":
            bn = int(module_def["batch_normalize"]) # batch_normalizationn is a flag to tell whetehr or not to perform batch norm
...
class Darknet(nn.Module):
    ...
    def __init__(self, config_path, is_encoder, img_size=416):
        super(Darknet, self).__init__()
        self.module_defs = parse_model_config(config_path)

        self.is_encoder = is_encoder
        self.hyperparams, self.module_list = create_modules(self.module_defs, self.is_encoder)
    ...
    def forward(self, x, targets=None):
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            # x is of shape (B, (5+num_classes)*3, 13,13)) / (B, (5+num_classes)*3, 26,26)) / (B, (5+num_classes)*3, 52,52)) 
                ...
                nG = x.shape[2]
                nA = 3
                p = x.view(nB, nA, 5+self.num_classes, nG, nG).permute(0, 1, 3, 4, 2).contiguous() #(B,3,13,13,(5+num_classes)) / (B,3,26,26,(5+num_classes)) / (B,3,52,52,(5+num_classes))
                p = p.reshape(nB, -1, self.num_classes + 5)
                
                if prediction is None:
                    prediction = p
                
                else:
                    prediction = torch.cat((prediction, p), dim = 1)
            ...
            # x is of shape (B, (5+num_classes)*3, 13*13)) / (B, (5+num_classes)*3, 26*26)) / (B, (5+num_classes)*3, 52*52)) 
                ...
                if is_training:
                    ...
                    y, *losses = module[0](y, targets) # WHy not module(x, targets)
                ...
                # Test phase: Get detections
                    ...
                    y = module(y)
                output.append(y) # List of output of the each layer if not training, otherwise appending losses of one batch after every YOLO layer
...
class Discriminator(nn.Module):
    def __init__(self, slope, n_classes, h=500):
        super(Discriminator, self).__init__()
        self.l1 = nn.Linear(10647*(n_classes + 5), 512)
        self.l5 = nn.Linear(512, 2)
        self.slope = slope
